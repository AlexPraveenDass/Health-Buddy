...
% and puts the list ofknowledge back in the right order
 ...
 *      that matters for this project. If this project was to be extended, this predicate could
 *      be redesigned to operate recursively on Rules.
 */
get_knowledge([], _, Knowledge, Knowledge):- !. % Stop condition, Rules = []
get_knowledge([(Fact if Condition)|Rules], Template, ExistingKnowledge, NewKnowledge):- % Fails is the rule has an incorrect representation
    (
